...
socketClient.on('chat:typing', (username) => {
    io.emit('displayTyping', `${username} está escribiendo...`);
});

socketClient.on('chat:stoppedTyping', (username) => {
    io.emit('hideTyping', username);
});
